...
if $USE_MULTITAIL; then
	RUST_BACKTRACE=full RUST_LOG_STYLE=always cargo run --bin cli $SPEED --features importer/scenarios -- import $@ > /tmp/abst_stdout 2> /tmp/abst_stderr &
	multitail /tmp/abst_stdout -cT ANSI /tmp/abst_stderr
else
	RUST_BACKTRACE=full cargo run --bin cli $SPEED --features importer/scenarios -- import $@
fi
