...
impl From<&Vec<String>> for Calendar {
    fn from(v: &Vec<String>) -> Self {
        Calendar { service_id:v[0].to_owned(),
            monday:v[1].to_owned(),
...
impl fmt::Display for Calendar {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\",\"{}\"", self.service_id, self.monday, self.tuesday, self.wednesday, self.thursday, self.friday, self.saturday, self.sunday, self.start_date, self.end_date)
    }
}
...
pub fn read_calendar(trips: &Vec<Trips>, path: String) -> Option<Vec<Calendar>> {
    let file = File::open(path).ok()?;
    let reader = BufReader::new(file);
    let mut calendar = reader.lines().into_iter().map(|line| {
        let v = line.unwrap().split(",").map(|word| word.replace("\"", "")).collect::<Vec<_>>();
        trips.iter().filter(|trip| v[0] == trip.service_id).map(|_service_id| Calendar::from(&v)).collect::<Vec<_>>()
